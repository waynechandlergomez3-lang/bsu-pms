...
const ParkingSpaceManagement = () => {
    ...
    const fetchData = async () => {
      try {
        ...
        const [layoutsResponse, driversResponse] = await Promise.all([
          axios.get('http://localhost:8000/api/parking-layouts'),
  ...
  useEffect(() => {
    if (selectedLayout) {
      const fetchAssignments = async () => {
        try {
          const response = await axios.get(`http://localhost:8000/api/parking-layouts/${selectedLayout.id}/assignments`);
  ...
  const handleAssignDriver = async (spaceId, driverId) => {
    try {
      await axios.post(`http://localhost:8000/api/parking-assignments`, {
        space_id: spaceId,
        driver_id: driverId
      });
  ...
  const handleUnassignDriver = async (spaceId) => {
    try {
      await axios.delete(`http://localhost:8000/api/parking-assignments/${spaceId}`);
