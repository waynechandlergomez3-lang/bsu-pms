...
import { FaPlus, FaMinus, FaExpand, FaCar, FaRotateRight, FaRotateLeft, FaCopy, FaDrawPolygon, FaCircle, FaArrowLeft, FaEye, FaEyeSlash } from 'react-icons/fa6';
        ...
        try {
            ...
            const processedLayout = {
                ...
                parking_slots: (layoutData.data.parking_slots || []).map(slot => ({
                    ...slot,
                    rotation: slot.rotation || slot.metadata?.rotation || 0,  // Try direct rotation first, then metadata
                    x_coordinate: slot.position_x || 0,
                    y_coordinate: slot.position_y || 0,
                    width: slot.width || 60,
    ...
    return (
        <div className={styles.pageContainer}>
            ...
            <div 
                ...
                onWheel={handleWheel}
            >
                <div className={styles.layoutWrapper} style={{ width: '1024px', height: '768px', margin: '0 auto', position: 'relative' }}>
                    <div 
                            ...
                            {layout.parking_slots.map(slot => {
                                console.log('Rendering slot:', {
                                    ...
                                    dimensions: {
                                        width: slot.width,
                                        height: slot.height,
                                        metadata_width: slot.metadata?.width,
                                        metadata_height: slot.metadata?.height
                                    },
                                    rotation: {
                                        direct: slot.rotation,
                                        metadata: slot.metadata?.rotation
                                    },
                                    position: {
                                        x: slot.x_coordinate,
                                        y: slot.y_coordinate,
                                        metadata_x: slot.metadata?.x,
                                        metadata_y: slot.metadata?.y
                                    }
                                });
                                ...
                                return (
                                    <div
                                        ...
                                        style={{
                                            ...
                                            backgroundColor: slot.metadata?.fill || 'rgba(0, 0, 0, 0.2)',
                            ...
                            {layout.parking_slots.map(slot => {
                                ...
                                return (
                                    <tr key={slot.id} className={slot.space_status === 'reserved' ? styles.reservedRow : 
                                                               ...
                                                               styles.availableRow}>
                                        <td>{slot.name || slot.space_number}</td>
                                        <td>
                                                    ...
                                                    onClick={async () => {
                                                        if (window.confirm('Are you sure you want to unassign this parking space?')) {
                                                            try {
                                                                const token = localStorage.getItem('token');
                                                                await axios.post(
                                                                    `http://localhost:8000/api/parking-assignments/${assignment.id}/end`,
                                                                    {},
                                                                    {
                                                                        headers: {
                                                                            'Authorization': `Bearer ${token}`,
